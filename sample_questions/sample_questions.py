...
sampled_posts_idx = np.random.choice(range(len(all_posts)), replace=False, size=1000, p=posts_prob)
...
with open('questions.sampled.csv', 'w') as csvfile:
    writer = csv.DictWriter(csvfile, fieldnames=['Id', 'Title', 'ViewCount', 'Score', 'Rank'])
    writer.writeheader()
